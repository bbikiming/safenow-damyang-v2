...
**관리 화면 (2026-07-29 신설 · 2026-07-30 탭 통합)** — 시스템 관리 > 법령 관리 **단일 화면**(`admin-law.html`)의 3탭에서 관리한다: 법령·조문(`DYADMLAW`) / 메뉴 근거 매핑(`DYADMLAWMAP`) / 변경 이력. 탭 셸은 `LAWTABS`(`js/admin-law.js`), 공용 코어는 `js/law-admin.js`(전역 `LAWADM`). 구 `admin-law-map.html` 은 매핑 탭 리다이렉트 스텁(쿼리 보존) — 별도 메뉴로 되돌리지 말 것. 매핑 탭의 미저장 변경은 탭 전환에도 confirm 가드가 걸린다.
