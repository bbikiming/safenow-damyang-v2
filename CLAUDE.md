...
- **시연 투어는 `RSKTOUR`(`js/rsk-tour.js`) 하나다 (2026-08-04 신설)** — 생성부터 온나라 이관까지 **9단계**를 ① 전체 흐름 보드(`openFlow()`)와 ② 단계별 가이드(`start()`)로 안내한다. `rsk-list`(진입 바 + 패널)와 `my-work`(패널만 — 여러 도메인이 모이는 화면이라 위험성평가 바를 상시 노출하지 않는다)에서 `boot()` 한다.
  ...
  - **앵커는 `data-tour` 속성으로만** (`rsk-create`·`rsk-survey`·`rsk-report`·`rsk-deliver`·`rsk-confirm`·`rsk-doc`·`mw-rskreport`·`mw-improve`). `render()` 가 `innerHTML` 을 통째로 갈아끼우므로 **DOM 참조를 보관하지 말 것** — 매번 새로 조회하는 idempotent `applyFocus()` 를 `MutationObserver(childList+subtree)` 가 부른다.
  ...
  - `js/edu-tour.js`(EDUTOUR)는 손대지 않는다 — CLAUDE.md §4 가 "발표용 핵심 자산 · 제거 금지"로 못박았고, rsk 2화면과 edu 9화면은 교집합이 0이라 두 엔진이 충돌하지 않는다.
