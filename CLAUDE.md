...
- 항목은 `{ question, now, why, options[], impact }` 로 적는다 — **질문만 있고 "지금은 어떻게 동작하는지"가 없으면 답할 수 없다.**
- **결정을 그 자리에서 기록한다** — 선택지 칩 + 메모가 `localStorage('dy-policy-v1')` 에 쌓이고, `[정리본 보기]` 가 회의록에 그대로 옮길 텍스트를 만든다. 기록은 항목 정의를 덮지 않는다.
