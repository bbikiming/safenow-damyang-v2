...
(function (global) {
    ...
    var ITEMS = [
        ...
        },
        {
            ...
            now: '수신 전까지 EVENT 단계 19건의 생성방식을 「판정 보류」로 두고, 이행 상세에 어느 인터페이스에서 무엇을 기다리는지 밝힙니다. 도급·용역 대상 목록이 없어 관리대상 단위의 공사·용역 축은 비워 둡니다 — 대상을 지어내지 않습니다. 담양군 확인(2026-08-31) — 연계 가능 여부 자체가 아직 미정입니다. 연계 전제를 화면에 만들지 않습니다.',
        ...
        },
        {
            ...
            now: '2025년 문서 원장에 부서 값이 실려 부서 조회·부서별 집계가 동작합니다. 다만 담당자(개인)는 여전히 없어 «기록 없음»으로 두고, 원장의 부서 구분이 시스템 조직도와 깊이가 달라(조직도의 「보건소」가 원장에서는 「보건소보건행정과」) 임의로 합치지 않고 나란히 보여줍니다. 조회를 부서로 좁히지는 않습니다 — 좁힐지는 담양군이 정할 일이고, 지금 좁히면 자기 부서 밖 이행 상황을 아무도 못 보게 됩니다.',
            options: [{ k: 'waiting', label: '규격 대기' }, { k: 'partial', label: '부분 연계' }, { k: 'received', label: '수신 완료' }]
        },
        {
            id: 'dept-duty', kind: 'data', menu: '업무 관리',
            title: '부서별 이행항목 소관 (어느 할 일을 어느 과가 맡는가)',
            now: '분류기준의 「적용대상」 문구에서 부서 이름을 찾아 추정하는데, 실측 결과 20개 부서 중 19개에서 «추정 밖인데 서류가 있는 할 일»이 추정 안쪽보다 많았습니다(추정 23~48개 vs 추정 밖 51~90개). 그래서 부서별 이행률을 «분모 확인 필요»로 두고 비율을 내지 않습니다 — 분모가 그만큼 틀린 비율을 칩으로 찍으면 담당자에게는 그것이 측정값이 됩니다. 대신 추정을 거치지 않은 두 수(서류가 붙은 할 일 · 보유 업무문서)만 실측으로 냅니다. 소관표를 받으면 그 자리가 실측 이행률로 바뀝니다.',
            options: [{ k: 'waiting', label: '자료 대기' }, { k: 'partial', label: '부분 자료' }, { k: 'received', label: '수신 완료' }]
        },
        {
