...
(function (global) {
    ...
    var MAP = {
        ...
        /* 업무 관리(현행) — 2026-09-03 추가. 종전에는 도움말이 **은퇴 화면에만** 있어,
           살아 있는 이행 관리·문서 목록에는 [?] 버튼이 뜨지 않고 도달할 수 없는
           업무 목록·이행 목록에만 도움말이 남아 있었다. */
        'cmp-status': {
            lead: '법으로 해야 하는 일 하나하나에 서류가 갖춰졌는지 봅니다. 비어 있는 칸이 곧 해야 할 일입니다.',
            tasks: [{ t: '문서로 찾기', go: 'cmp-docs.html' }, { t: '내 할일 보기', go: 'my-work.html' }],
            tour: 'DOCTOUR'
        },
        'cmp-docs': {
            lead: '우리 부서가 주고받은 문서를 조건으로 좁혀 찾습니다. 어떤 할 일의 증빙인지도 함께 봅니다.',
            tasks: [{ t: '할 일별로 보기', go: 'cmp-status.html' }, { t: '내 할일 보기', go: 'my-work.html' }],
            tour: 'DOCTOUR'
        },
        /* 아래 둘은 메뉴에서 뺀 화면이다(주소로만 열린다). 도움말을 지우지 않는 이유는
           정의서를 남기는 이유와 같다 — 링크·즐겨찾기로 도착하는 사람이 있다.
           다만 **바로가기는 현행 화면을 가리킨다** — 은퇴 화면끼리 돌리지 않는다. */
        'docs-preset': {
            lead: '이전 버전 화면입니다. 지금 문서를 찾는 자리는 업무 관리 > 문서 목록입니다.',
            tasks: [{ t: '문서 목록으로', go: 'cmp-docs.html' }],
            tour: 'DOCTOUR'
        },
        'docs-exec': {
            lead: '이전 버전 화면입니다. 지금 할 일별 증빙을 보는 자리는 업무 관리 > 이행 관리입니다.',
            tasks: [{ t: '이행 관리로', go: 'cmp-status.html' }],
            tour: 'DOCTOUR'
        },
        'work-admin': {
        ...
        /* 관리대상(2026-09-03 개명·축소·복원) — 이 축의 화면에 도움말이 없었다 */
        'base-targets': {
            lead: '담양군이 다루는 원료·제조물 품목을 등록하고, 품목마다 안전자료를 갖고 있는지 확인합니다.',
            tasks: [{ t: '시설물 대장으로', go: 'fac-list.html' }]
        },
        'fac-list': {
            lead: 'FMS에서 받은 시설물 대장을 찾아보고, 위험성평가에 필요한데 대장에 없는 값을 채웁니다.',
            tasks: [{ t: '위험도 순으로 보기', go: 'fac-risk.html' }]
        },
        'fac-risk': {
            lead: '이용자 안전 관점에서 점검 순서를 정합니다. 값이 모자라 순위를 못 매긴 시설물은 보완입력으로 채웁니다.',
            tasks: [{ t: '시설물 대장으로', go: 'fac-list.html' }]
        },
