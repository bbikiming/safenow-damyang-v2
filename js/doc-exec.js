...
(function (global) {
    ...
    /* 연도별 자료의 성격 — 2025 는 문서 원장 실측, 2026 은 규칙으로 만든 예시 자료다.
       같은 화면에서 연도만 바꾸면 성격이 통째로 달라지므로 밝히지 않으면
       «올해 이만큼 했다»로 읽힌다(이행 관리 화면과 같은 문장을 쓴다). */
    function seedNote() {
        var sd = global.DYDOC2026;
        if (!sd || !sd.META || S.year !== +sd.META.year) return '';
        return '<p><b>' + S.year + '년 실적은 예시 자료입니다.</b> ' + D().BASE_YEAR +
            '년 실측 문서 원장에서 «작년에 실적이 있던 할 일만, 도래한 회차 안에서» 규칙으로 만든 ' +
            '자료이고(단계 ' + sd.META.stagesSeeded + '개 · 문서 ' + sd.META.docs + '건) 실제 제출 기록이 아닙니다.</p>';
    }
    function notice() {
        ...
        var lead = nextStep();
        /* 흐름 설명(올리면 진행중 → 확인해야 완료)은 위 첫 방문 배너가 이미 한다.
           같은 말을 두 번 하면 띠만 늘고 목록이 밀린다. 여기는 **다른 화면과의
           구분**만 맡는다 — 첫 방문 배너에 없는 정보다. */
        /* «완료 0 · 진행중 179» 를 그대로 두면 «아무것도 안 끝났다»로 읽힌다.
           이행 관리는 같은 179개를 «이행»이라 부르는데, 둘은 **다른 것을 재고
           있다** — 저기는 «법으로 해야 할 일을 했는가», 여기는 «그 서류를 우리가
           확인했는가». 축이 다르다는 것을 화면이 말하지 않으면 한 시스템이 같은
           수를 두 가지로 말하는 것으로 보인다(§5 «이행을 말하는 축이 셋»). */
        var s0 = D().summary(S.year);
        var rest =
            '<p><b>여기 「완료」는 법정 이행 여부가 아니라 우리가 증빙을 확인했는지입니다.</b> ' +
            '서류가 올라오면 <b>진행중</b>이고, 재난안전과 담당자가 확인해야 <b>완료</b>가 됩니다. ' +
            '법으로 해야 할 일을 했는지는 <a href="cmp-status.html?year=' + S.year + '">업무 관리 &gt; 이행 관리</a>가 판정하며, ' +
            '지금 진행중 ' + s0.counts.in_progress + '건은 그쪽에서 <b>이행</b>으로 잡힙니다 — ' +
            '문서 원장에서 올라온 서류라 아직 아무도 확인하지 않은 상태입니다.</p>' +
            ...
            seedNote();
        /* 첫 방문 배너가 흐름을 설명하므로 안내는 기본 접힘 — 접혀도 '지금 할 일'은 남는다 */
